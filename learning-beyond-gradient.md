...
- Atari57 我也尝试跑了一整套无人值守实验：`57 个游戏 x 2 种输入 x 3 次运行 = 342 条 coding-agent 搜索轨迹`。表现有好有坏，但按每个游戏取 best input 的口径，Codex median HNS 是 `0.83`，已经和 OpenAI Baselines PPO2 的 `0.80`、CleanRL EnvPool PPO 的 `0.98` 在同一档；如果取 best single run，median HNS 是 `1.18`。
...
先看环境步曲线。在完全无人工介入的批量运行里，`native_obs` 到 `9.7M` 步附近的 Atari median HNS 是 `0.81`，`ram` 是 `0.59`。同一张对比里，OpenRL Benchmark 保存的 PPO2 / CleanRL EnvPool PPO median HNS 曲线到 `10M` 步大约是 `0.88 / 0.92`。
